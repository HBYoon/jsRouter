...
// 130921 - start writing
// 130924 - jsRouter 0.0.1
// 130925 - runToState/run Update
// 131214 - runToState update -> add state.$.fix
// 140119 - jsonTag object update -> add created jsonTag object for page init run
// 140120 - jcss cssMain rewrite -> use array and join for string handling
// 140121 - run update -> work sequence change
...
// DOM2 event model
// window.history
// Worker / for fast async html tamplate
...
var jsRouter = function jsRouter (debug) {
  ...
  var jsonTagObj = {
    ...
    create: function(opt){
      ...
      jsonTag.toSyncTime = opt.toSyncTime || 50;
  ...
  var setFatalErrorHandling = function(jr){
    jr.data.error.fatal = jr.data.error.fatal || (dummyFactory('fatal'));
    jr.top.on('error',function(err){
      if (err.fatal) {
        jr.run(jr.data.error.fatal.$);
      }
    });
  };
  ...
  */
  jsRouter.runToState = function(targetPath, replaceState){
    ...
    if (!(state && state.$)) {
      ...
      return self.run(self.data.error['404'].$);
    }
  ...
  jsRouter.create = function(opt){
    ...
    jr.data.error = jr.data.error || {};
    jr.data.error['404'] = jr.data.error['404'] || (dummyFactory('404'));
